...
Thread::Thread(char* threadName, int threadID)
{
    ...
    space = NULL;
    thread_priority = 0;
    approximate_burst_time = 0;
    burst_time = 0;
    start_time = 0;
    thread_age = 0;
    waiting_time = 0;
    start_waiting_time = 0;
    remain_app_burst_time = 0;
}
...
Thread::Yield ()
{
    ...
    if(this->thread_priority <= 49 || this->thread_priority >= 100){
        this->Update_Burst_Time();
...
Thread::Sleep (bool finishing)
{
    ...
    //DEBUG(dbgMP3, "/n" <<kernel->stats->totalTicks << "   " << this->start_time << "    " << kernel->stats->totalTicks - this->start_time);
    //DEBUG(dbgMP3, this->burst_time);
    this->burst_time += kernel->stats->totalTicks - this->start_time;
    //DEBUG(dbgMP3, this->burst_time);
    ...
    //DEBUG(dbgMP3, "[E] Tick [" << kernel->stats->totalTicks << "]: Thread [" << nextThread->getID() << "] is is now selected for execution, thread [" << this->getID() << "] is replaced, and it has executed [" << this->burst_time << "] ticks");
    //DEBUG(dbgMP3, this->approximate_burst_time << "  " << nextThread->approximate_burst_time);

    double original_app_burst_time = this->approximate_burst_time;
    this->Approximate_Next_Burst_Time();
    //this->remain_app_burst_time = this->approximate_burst_time;
    DEBUG(dbgMP3, "[D] Tick [" << kernel->stats->totalTicks << "]: Thread [" << this->getID() << "] update approximate burst time, from: [" << original_app_burst_time << "], add [" << this->burst_time << "], to [" << this->approximate_burst_time << "]");
    //this->Reset_Burst_Time();

    kernel->scheduler->Run(nextThread, finishing, TRUE); 
}
...
// reset CPU burst time to 0(if I/O and next CPU burst)
void Thread::Reset_Burst_Time(){
    this->burst_time = 0;
}

// set the time that the thread starts at
void Thread::Set_Start_Time(){
    this->start_time = kernel->stats->totalTicks;
}

void Thread::Update_Burst_Time(){
    int rst = kernel->stats->totalTicks - this->start_time;
    this->burst_time += rst;
}

void Thread::Approximate_Next_Burst_Time(){
    int rst = 0.5 * this->burst_time;
    rst += 0.5 * this->approximate_burst_time;
    this->approximate_burst_time = rst;
}
