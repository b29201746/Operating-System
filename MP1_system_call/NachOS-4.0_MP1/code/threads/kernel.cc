...
Kernel::Kernel(int argc, char **argv)
{
    ...
    // initialize physical pages counter
    for(int i = 0; i < NumPhysPages; i++){
        UsedPhyPages[i] = false;
    }
    for (int i = 0; i < 10; i++) {
        execfile_priority[i] = 0;
    }
    FreePhyPageNum = NumPhysPages;
#ifndef FILESYS_STUB
    formatFlag = FALSE;
#endif
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (strcmp(argv[i], "-rs") == 0) {
            ...
            execfile[++execfileNum] = argv[++i];
            execfile_priority[execfileNum] = atoi(argv[++i]);
        } 
...
int Kernel::Exec(char* name, int pri)
{
    ...
    t[threadNum]->thread_priority = pri;
	threadNum++;
