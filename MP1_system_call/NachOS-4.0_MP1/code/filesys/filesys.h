...
//  The OpenAFile function is used for kernel open system call
    OpenFileId OpenAFile(char *name) {
        int flag = 1;  // flag = 1 if there is a empty space for OpenFileTable
        int idx = 0;  // if there is a empty space, record the index
        for(int i = 0; i < 20; i++){
        ...
        if(flag){
            OpenFile* toOpen = Open(name);
            if(toOpen != NULL){
                OpenFileTable[idx] = toOpen;
                return idx;
            }
            else
                return -1;
